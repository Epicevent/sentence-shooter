...
TOEFL Build-a-Sentence의 문맥 판단과 어구 배열을 회피 슈팅으로 훈련하는 정적 Canvas 게임이다. 현재 `torus-26`은 통합안 C를 공통 기반으로 삼고 수동 STORM의 조향 방식만 A/B로 비교한다.
...
쿼리 없는 주소와 `?ab=C`는 v25 통합 control이다. `?ab=A`는 방향 고정 STORM, `?ab=B`는 재조향 STORM 후보이며 둘 다 C의 조립·열역학·보상 규칙을 사용한다.

문자를 입력하면 일치하는 모든 단어 상자가 함께 포커스된다. 입력을 더해 후보를 좁히고, 후보가 하나일 때 Tab으로 확정한다. 완성된 짧은 청크와 더 긴 접두사 청크가 겹치면 완전일치가 우선된다. Tab 아이템과 실드 아이템은 없다. 좌우 화살표로 이동하고, A/B에서는 실물 명중 3회 뒤 `Shift+←/→`로 STORM을 시전한다. Space는 타자 리듬을 방해하지 않도록 아무 스킬에도 연결하지 않는다. 정답 논리 판정은 즉시 진행되지만 단어의 폭발·점수 회수·보상 생성은 예약 미사일의 실제 마지막 충돌 순간에만 일어난다.
...
- A/B에서는 같은 실물 명중이 STORM charge를 한 칸 채운다. 3칸이 되면 좌우 방향을 골라 한 개의 장을 수동 시전한다. 장은 초당 34px로 천천히 움직이며 수평 토러스 경계를 이어 간다. A는 방향이 고정되고 B는 활성 중에도 방향을 바꿀 수 있다. C control만 기존 이동 자동시전을 유지한다.
...
- A `DRIFT STORM`: 시전할 때 정한 방향으로 장이 끝까지 흐른다. 활성 중 반대 방향 입력은 소비 없이 거절된다.
- B `STEER STORM`: 같은 충전량·반경·수명·냉각력으로 시작하지만 활성 중 `Shift+←/→`로 흐름을 반전할 수 있다.

A/B의 차이는 STORM 방향 제어뿐이다. 레일/코어 조립, 오답, 점수, 호위기, 열 계산은 같다.
...
로컬 게임오버 또는 페이지 이탈 시 pipeline-8 세션이 `traces/tr_*.json`에 저장된다.
...
분석기는 문항 속도, 논리 판정→물리 명중 지연, 온도를 보존하는 오답 탄막, 제한 요격 사용, 움직이는 STORM, 확장 냉각의 전후 열량, 국소 바닥 온도와 열 커널 시간을 함께 출력한다. `npm test`는 정확 접두사 포커스, 두 조립 경로, 4기 호위기의 실제 평타 발사, A/B 수동 STORM, 오답 전후 온도 불변, 실물 냉각, swept collision, typed-array 재사용, 70문항과 trace 저장을 검증한다. `npm run verify:play`는 `reviewer=agent-storm-a`/`reviewer=agent-storm-b` 현재 build 원본의 최소 인과 증거만 확인하며 재미를 판정하지 않는다.
