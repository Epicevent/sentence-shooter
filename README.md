...
TOEFL Build-a-Sentence의 문맥 판단과 어구 배열을 회피 슈팅으로 훈련하는 정적 Canvas 게임이다. 현재 `torus-28`은 1945식 격납고에서 성공 동사가 다른 네 기체를 선택한다.
...
http://127.0.0.1:7777/game?craft=striker&seed=20260728
http://127.0.0.1:7777/game?craft=phantom&seed=20260728
http://127.0.0.1:7777/game?craft=carrier&seed=20260728
http://127.0.0.1:7777/game?craft=bulwark&seed=20260728
```

쿼리 없는 주소에서는 격납고가 마지막 선택 기체를 기억한다. `?craft=`는 시작 카드를 명시한다. 과거 `?ab=A/B` 링크는 각각 STRIKER/PHANTOM 사전선택으로만 남아 있다.

문자를 입력하면 일치하는 모든 단어 상자가 함께 포커스된다. 입력을 더해 후보를 좁히고, 후보가 하나일 때 Tab으로 확정한다. 완성된 짧은 청크와 더 긴 접두사 청크가 겹치면 완전일치가 우선된다. Tab 아이템과 실드 아이템은 없다. 좌우 화살표로 이동한다. STRIKER만 실물 명중 3회 뒤 `Shift+←/→`로 BIG WING SWEEP을 시전한다. Space는 타자 리듬을 방해하지 않도록 아무 스킬에도 연결하지 않는다. Tab 확정은 즉시 50ms 타격 피드백을 주지만 단어의 폭발·점수 회수·기체 보상은 예약 미사일의 실제 마지막 충돌 순간에만 일어난다.

한 출격은 일반 문장 3개와 8청크 보스 문장 1개로 구성된다. 마지막 청크를 확정하면 다음 편대가 회색 비활성 상태로 먼저 진입하고, 문장 완료는 1.8초 동안 완성문과 오답 교정을 보여 준 뒤 이어진다.

문항 풀은 기존 12문항, 교재 사진에서 전사한 58문항, 8청크 보스 4문항으로 총 74문항이다. 같은 seed의 네 기체는 문항·어구·초기 배치·판정을 공유한다.
...
오답이 확정되면 화면의 노란 열안개를 내부 표본화하던 위치에서 220ms 예고를 가진 붉은 탄막이 응축된다. 탄은 열장의 복제된 위협 표현이며 권위 온도장과 tracer를 소비하지 않는다. 따라서 일부러 틀려도 세계 온도·안개·바닥 열은 내려가지 않는다. 네 기체 모두 승인 기준선인 원형 실루엣과 속도 반대 방향 11px 꼬리를 사용한다. 탄 수명은 `예고 + 발사 거리/속도 + 통과 여유`이므로 먼 탄도 조준점을 통과하기 전에 사라질 수 없다. 첫 피격은 맞은 한 발만 없애고 나머지 탄막을 보존한다. i-frame 접촉으로 나머지를 지우지 않으므로, 가만히 있으면 약 1초 간격의 연속 피격으로 실제 게임오버까지 이어진다.
...
- `STRIKER`: 실물 정답 명중마다 호위기가 좌우 2기씩 최대 4기까지 붙고 실제 평타 발사점이 늘어난다. 3회 명중으로 600px/s BIG WING SWEEP을 쓰고 `Shift+←/→`로 남은 항적을 움직인다.
- `PHANTOM`: 오답과 무관한 압박탄을 안전하게 스치면 SYNC가 25씩 찬다. 100%에서 자동이 아닌 정답의 실제 명중이 화면의 모든 적탄을 코어로 곡선 흡수한다. 오답탄은 그레이즈 자원이 아니며 오답은 SYNC를 잃는다.
- `CARRIER`: 정답 청크를 실제 명중 시 포획한다. 다음 입력은 반대편 점멸 도크에 화물을 운반해 점수·조립을 정산할 때까지 잠긴다. 운반 중 화면 클릭/탭은 요격탄이다.
- `BULWARK`: 실제 정답 위치에 11초 세로 반사선을 남긴다. 함선이 선 위에 있을 때 선을 가로지르는 탄만 반사·환전한다. 오답은 모든 선을 붕괴시킨 뒤 기존 붉은 탄막을 무장한다.

문장 clear는 방사형 quench를 발동해 front가 통과한 온도를 52% 낮춘다. 1,500점마다 발동하는 `SCORE BREAK`는 현재 붉은 탄막을 지우고 88% 냉각 front를 발동한다. 살아 있는 단어 경계는 다음 열 스텝에서 다시 열원으로 clamp된다.
...
로컬 게임오버 또는 페이지 이탈 시 pipeline-10 세션이 `traces/tr_*.json`에 저장된다.
...
분석기는 문항 속도, 논리 판정→물리 명중 지연, 온도를 보존하는 오답 탄막, 네 기체 상태, 확장 냉각의 전후 열량, 국소 바닥 온도와 열 커널 시간을 함께 출력한다. `npm test`는 격납고, 네 성공 동사, 즉시 Tab 피드백, 정확 접두사 포커스, 4기 호위기의 실제 평타, 스트리밍 소티와 보스, 오답 전후 온도 불변, 실물 냉각, swept collision, typed-array 재사용, 74문항과 trace 저장을 검증한다. `npm run verify:play`는 두 독립 플레이어가 네 기체로 만든 현재 build 원본의 최소 인과 증거만 확인하며 재미를 판정하지 않는다.
